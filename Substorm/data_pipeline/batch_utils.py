...
import pandas
pandas.options.mode.chained_assignment = None  # default='warn'. This avoids the warning 'SettingWithCopyWarning: A value is trying to be set on a copy of a slice from a DataFrame'.
import datetime
# import feather
...
class DataUtils(object):
    ...
    def _load_onset_data(self, northData, southData,\
        ...
        if self.loadPreComputedOnset:
            ...
            if self.useSML:
                print('creating onset data using create_sml_bins')
                onsetDF = dataObj.create_sml_bins(saveBinData=saveBinData,\
                                     saveFile=onsetSaveFile)
            else:
                print('creating onset data using create_output_bins')
